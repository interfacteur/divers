...
;(function($) {
    ...
    $.extend($.superbox, {
        ...
        open: function(content, curSettings) {
            ...
            /*
				<script> won't be cloned in the html so :
				use <object> instead of <script> to call js functions in your html page
			*/
			eval($innerbox.find('object.script').text()); //execute
			//$innerbox.find('object.script').remove(); //and remove from html

            
            var $objecttag = $innerbox.find('div.scriptsrc');
            if($objecttag){
                var $objectsrc = $objecttag.attr('rel'); 
                
                $scripttmp = $('<script></script>').attr('type', 'text/javascript').attr('src', $objectsrc);
                $objecttag.append($scripttmp);
            }
            
            
            //document.write('<scr'+'ipt type="text/javascript" s'+'rc="' + $objectsrc + '"></scr'+'ipt>');
            
            
			//$innerbox.find('object.hidden, div.script').remove(); //and remove from html
        ...
        image: function($elt, curSettings, isGallery) {
            ...
            $elt.click(function() {
                ...
                $.superbox.wait(function() {
                    ...
                    // Image
                    var $curImg = $('<img src="'+ $elt.attr("href") +'" alt="" class="'+ $elt.attr("class") +'" id="'+ $elt.attr("id") +'" /><div class="legend"><p>'+($elt.find('.legend').text())+'</p></div>');
    ...
    function showBox(curSettings) {
        ...
        // Show $superbox
        $superbox.css({position: "relative", top: 0, opacity: 0});//was "static"
    ...
    function createElements() {
        ...
        // Add close button
        $closeBtn = $('<p class="close"><a tabindex="0" role="button" href="#" class="superbox-close">'+ settings.closeTxt +'</a></p>').appendTo($superbox).find("a");
